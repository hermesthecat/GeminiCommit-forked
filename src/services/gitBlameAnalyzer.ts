...
export class GitBlameAnalyzer {
        ...
        lines.forEach((line: string) => {
          if (line.startsWith("author ")) {
            ...
            currentBlame.date = new Date(
              parseInt(line.substring(15)) * 1000
            ).toISOString();
  ...
  private static async getDiff(
    ...
    return new Promise((resolve, reject) => {
      const gitProcess = spawn("git", ["diff", "--", path.basename(filePath)], {
        cwd: path.dirname(filePath),
      });
      let diff = "";
  ...
  private static analyzeBlameInfo(
    ...
    changedLines.forEach((lineNumber) => {
      ...
      if (blameInfo) {
        const authorData = authorChanges.get(blameInfo.author) || {
          count: 0,
          lines: [],
        };
        authorData.count++;
  ...
  private static formatAnalysis(
    analysis: Map<string, { count: number; lines: number[] }>
  ): string {
...
export async function analyzeFileChanges(filePath: string): Promise<string> {
  ...
  try {
    return await GitBlameAnalyzer.analyzeChanges(
      workspaceFolder.uri.fsPath,
      filePath
    );
  } catch (error) {
    console.error(`Error analyzing file changes for ${filePath}:`, error);
    return `Unable to analyze changes for ${filePath}: ${
