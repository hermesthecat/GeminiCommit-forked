...
export class ApiKeyValidator {
  private static readonly geminiTestEndpoint =
          ...
          switch (axiosError.response.status) {
            ...
            default:
              throw new AiServiceError(
                `API validation failed: ${axiosError.response.status}`
              );
          }
        }
      }
  ...
  static async validateCustomApiKey(
    key: string,
    endpoint: string
  ): Promise<boolean> {
    if (!this.isKeyFormatValid(key)) {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response) {
          switch (axiosError.response.status) {
            ...
            default:
              throw new AiServiceError(
                `Custom API validation failed: ${axiosError.response.status}`
              );
          }
        }
      }
  ...
  private static isKeyFormatValid(key: string): boolean {
    ...
    if (key.length < this.minKeyLength) {
      void Logger.error(
        `API key must be at least ${this.minKeyLength} characters long`
      );
      return false;
    }
