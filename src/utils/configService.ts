...
export class ConfigService {
  ...
  static getConfig<T extends CacheValue>(
    ...
    try {
      ...
      if (!this.cache.has(cacheKey)) {
        ...
        void Logger.log(
          `Config loaded: ${cacheKey} = ${JSON.stringify(value)}`
        );
      }
      return this.cache.get(cacheKey) as T;
    } catch (error) {
      void Logger.error(
        `Error getting config ${section}.${key}:`,
        error as Error
      );
      return defaultValue;
    }
  ...
  static async setApiKey(key: string): Promise<void> {
    try {
      ...
      await vscode.window.showInformationMessage(
        "Google API key has been successfully validated and saved"
      );
    } catch (error) {
      void Logger.error(
        "Failed to validate and set Google API key:",
        error as Error
      );
      await vscode.window.showErrorMessage(
        `Failed to set API key: ${(error as Error).message}`
      );
      throw error;
    }
  }
  ...
  static async getCustomApiKey(): Promise<string> {
    try {
      ...
      throw new AiServiceError(
        "Failed to get custom API key: " + (error as Error).message
      );
    }
  }
  ...
  static async setCustomApiKey(key: string): Promise<void> {
    try {
      ...
      await vscode.window.showInformationMessage(
        "Custom API key has been successfully validated and saved"
      );
    } catch (error) {
      void Logger.error(
        "Failed to validate and set custom API key:",
        error as Error
      );
      await vscode.window.showErrorMessage(
        `Failed to set custom API key: ${(error as Error).message}`
      );
      throw error;
    }
  }
  ...
  static async removeCustomApiKey(): Promise<void> {
    try {
      ...
      await vscode.window.showInformationMessage(
        "Custom API key has been removed"
      );
    } catch (error) {
  ...
  static getCommitLanguage(): string {
    ...
    const configLang = this.getConfig<string>(
      "commit",
      "commitLanguage",
      "english"
    );
